...
def post_detail(request, post_pk):
    ...
    # ModelManager의 get메서드를 이용해서 단 한개의 객체만 가져온다
    ...
    # ! 이 뷰에서는 render를 사용하지 않고 전체과정loader, HttpResponse)을 기술
    # Django가 템플릿을 검색 할 수 있는 모든 디렉토리를 순회하며
    # 인자로 주어진 문자열값과 일치하는 템플릿이 있는지 확인 후,
    # 결과를 리턴 (django.template.backends.django.Template클래스형 객체)
    template = loader.get_template('post/post_detail.html')
    # dict형 변수 context의 'post'키에 post(Post)객체를 할당
    context = {
    ...
    # 반환된 string을 HttpResponse형태로 돌려준다.
