...
def youtube_search(request):
    ...
    # 1. 유투브 영상을 젖아할 class Video(models.Model)생성
    # 2. 검색결과의 videoId를 Video의 youtube_id필드에 저장
    #    해당 필드는 unique해야 함
    # 3. 검색결과에서 videoId가 Video의 youtube_id와 일치하는 영상이 이미 있을 경우에는 pass,
    #    없을 경우 새 Video객체를 만들어 DB에 저장
    # 4. 이 후 검색결과가 아닌 자체 DB에서 QuerySet을 만들어 필터링한 결과를 템플릿에서 표시

    # [2] 위 과제로 완성된 검색결과에서 '포스팅하기버튼을 구현, Post가 Youtube영상을 포함하도록 함
    #    검색결과에서 '포스팅하기버튼을 누르면 해당 Video와 연결된 Post를 생성
    ...
    if q:
        ...
        # ex) 자체 DB쿼리
        # videos = Video.objects.filter(Q(title__contains=q))
        # context = {
        #     'videos': videos,
        # }
    else:
        # 1. search list API의 주소로 GET요청을 보냄
